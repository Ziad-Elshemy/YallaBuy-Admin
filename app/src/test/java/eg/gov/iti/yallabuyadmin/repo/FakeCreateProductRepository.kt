...
class FakeCreateProductRepository: Repository {

    override suspend fun getAllVendors(): Flow<ProductsResponse?> {
        return flowOf(
            ProductsResponse(
                products = listOf(
                    ProductsItem(vendor = "Nike"),
                    ProductsItem(vendor = "Adidas")
                )
            )
        )
    }
    ...
    override suspend fun getAllProductTypes(): Flow<ProductsResponse?> {
        return flowOf(
            ProductsResponse(
                products = listOf(
                    ProductsItem(productType = "Shoes"),
                    ProductsItem(productType = "T-Shirts")
                )
            )
        )
    }
    ...
    override suspend fun createProduct(product: ProductsItem): Flow<ProductsItem?> {
        return flowOf(product.copy(id = 10L))
    }

    override suspend fun assignProductToCollection(productId: Long, collectionId: Long): Flow<Unit> {
        return flowOf(Unit)
    }

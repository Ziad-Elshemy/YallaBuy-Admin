...
class FakeCreateDiscountRepository: Repository {

    override suspend fun getAllPriceRules(): Flow<List<PriceRulesItem>> {
        return flowOf(
            listOf(
                PriceRulesItem(id = 1L, title = "Summer Sale"),
                PriceRulesItem(id = 2L, title = "Winter Sale")
            )
        )
    }

    override suspend fun createDiscountCode(ruleId: Long, discountCode: DiscountCode): Flow<DiscountCode> {
        return flowOf(discountCode.copy(id = 1001L, priceRuleId = ruleId))
    }

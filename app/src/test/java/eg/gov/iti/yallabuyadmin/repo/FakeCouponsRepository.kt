...
class FakeCouponsRepository: Repository {

    override suspend fun getAllPriceRules(): Flow<List<PriceRulesItem>> {
        return flowOf(listOf(PriceRulesItem(id = 1, title = "Test Rule")))
    }

    override suspend fun getAllDiscountCodes(): Flow<List<DiscountCode>> {
        return flowOf(listOf(DiscountCode(id = 1, code = "SUMMER", priceRuleId = 1)))
    }
    ...
    override suspend fun deleteDiscountCode(priceRuleId: Long, discountCodeId: Long): Flow<Boolean> {
        return flowOf(true)
    }

    override suspend fun updateDiscountCode(priceRuleId: Long, discountCodeId: Long, discountCode: DiscountCode): Flow<DiscountCode> {
